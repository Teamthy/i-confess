...
func (h *Handler) Routes() http.Handler {
	...
	h.route(mux, "POST /schedules/{id}/start", "user", "schedules", "Trigger a schedule — builds a session", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.triggerSchedule)).ServeHTTP(w, r) })
	...
	h.route(mux, "POST /v1/sessions", "user", "sessions", "Compose a session; audio URLs are signed", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.createSession)).ServeHTTP(w, r) })
	...
	h.route(mux, "POST /v1/sessions/{id}/start", "user", "sessions", "Start a session (DRAFT→ACTIVE)", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.startSession)).ServeHTTP(w, r) })
	...
	h.route(mux, "POST /v1/sessions/{id}/complete", "user", "sessions", "Complete a session", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.completeSession)).ServeHTTP(w, r) })
	h.route(mux, "GET /v1/sessions/{id}/queue", "user", "sessions", "Queue for a session", authed, h.getSessionQueue)
	h.route(mux, "POST /v1/sessions/{id}/progress", "user", "sessions", "Sync playback progress", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.syncProgress)).ServeHTTP(w, r) })
	...
	h.route(mux, "POST /v1/schedules/{id}/start", "user", "schedules", "Trigger a schedule — builds a session (deep link target)", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.triggerSchedule)).ServeHTTP(w, r) })
	...
	h.route(mux, "POST /v1/templates/{id}/start", "user", "templates", "Start a template — builds a session", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.startTemplate)).ServeHTTP(w, r) })
	...
	h.route(mux, "POST /v1/subscriptions/verify", "user", "subscription", "Verify store receipt (server-side, billing.Verifier)", authed, func(w http.ResponseWriter, r *http.Request) { idempotent(http.HandlerFunc(h.verifySubscriptionV2)).ServeHTTP(w, r) })
	// Search (§41)
