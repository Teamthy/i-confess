...
func (h *Handler) createCommunityPost(w http.ResponseWriter, r *http.Request) {
	var req struct{ Body, Visibility string `json:"body"` }
	// support both keys
	...
	if v, ok := raw["body"].(string); ok { req.Body = v }
	if v, ok := raw["visibility"].(string); ok { req.Visibility = v }
	if req.Body == "" || len(req.Body) > 2000 {
		httpx.WriteError(w, http.StatusBadRequest, "body 1..2000 chars")
		return
	}
	if req.Visibility == "" { req.Visibility = community.VisibilityPrivate }
