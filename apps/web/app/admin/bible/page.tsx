...
export default function BibleAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [o, h, m, c, p, a, v] = await Promise.all([
        apiRequest("/admin/bible/overview", token), apiRequest("/admin/bible/health", token),
        apiRequest("/admin/bible/metrics", token), apiRequest("/admin/bible/catalog", token),
        apiRequest("/admin/bible/plans", token), apiRequest("/admin/bible/audio", token),
        apiRequest("/admin/bible/verse-of-day", token),
      ]);
      setOverview(o); setHealth(h); setMetrics(m); setCatalog(c.translations ?? []);
      setPlans(p.plans ?? []); setAudio(a.assets ?? []); setVerseDays(v.entries ?? []);
