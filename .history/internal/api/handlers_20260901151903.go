...
func (h *Handler) changePassword(w http.ResponseWriter, r *http.Request) {
	...
	userID := h.userID(r)
	u, hash, err := h.users.ByEmail(r.Context(), "")
	...
	u, hash, err = h.users.ByEmail(r.Context(), u.Email)
	if err != nil {
...
// adminSetSubscription updates a user's subscription plan and status.
func (h *Handler) adminSetSubscription(w http.ResponseWriter, r *http.Request) {
	...
	if !isValidSubscriptionPlan(req.Plan) {
		httpx.WriteError(w, http.StatusBadRequest, "invalid subscription plan")
		return
	}

	if !isValidSubscriptionStatus(req.Status) {
		httpx.WriteError(w, http.StatusBadRequest, "invalid subscription status")
		return
	}
	...
	httpx.WriteJSON(w, http.StatusOK, map[string]string{"message": "subscription updated successfully"})
}
