...
func (s *VoiceRightsStore) Create(ctx context.Context, vr *models.VoiceRights) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO voice_rights (id, voice_id, owner_name, license_status, commercial_use, ai_generation_allowed, 
		                            territories, languages, marketing_allowed, expiration_date, revocation_terms, 
...
func (s *VoiceRightsStore) ByVoiceID(ctx context.Context, voiceID string) (*models.VoiceRights, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, voice_id, owner_name, license_status, commercial_use, ai_generation_allowed, 
		        COALESCE(territories, ''), COALESCE(languages, ''), marketing_allowed, COALESCE(expiration_date, ''), 
